...
.PHONY: vs2019_uwp generate
# Define the vs2019_uwp target
vs2019_uwp: generate
...
ifdef MSBUILD
	$(SILENT) "$(MSBUILD_EXE)" "$(PROJECTDIR_WIN)/vs2019-winstore82/$(PROJECT_NAME).sln" $(MSBUILD_PARAMS)
endif
...
ifdef MSBUILD
	$(SILENT) "$(MSBUILD_EXE)" "$(PROJECTDIR_WIN)/vsllvm/$(PROJECT_NAME).sln" $(MSBUILD_PARAMS)
endif
